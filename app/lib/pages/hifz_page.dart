...
class _HifzPageState extends State<HifzPage> {
  ...
  // Send audio to Hugging Face API with retry mechanism
  Future<void> _processAudioWithAPI() async {
    ...
    // Read file as bytes (outside the retry loop to avoid repeated file reads)
    final File audioFile = File(_recordingPath!);
    final List<int> audioBytes = await audioFile.readAsBytes();
    
    await _processWithRetry(audioBytes);
  }
  
  // Helper method to handle API call with retry logic
  Future<void> _processWithRetry(List<int> audioBytes) async {
    try {
      // Prepare API request
      final headers = {
      ...
      while (!success && _currentRetry < _maxRetries) {
        setState(() {
          _apiResult = _currentRetry == 0 
              ? "Processing audio..." 
              : "Processing audio... (Attempt ${_currentRetry + 1}/${_maxRetries})";
        });
        
        debugPrint("API attempt ${_currentRetry + 1}: Sending ${audioBytes.length} bytes to Tarteel AI Whisper Quran model");
        ...
        try {
          ...
          if (response.statusCode == 200) {
            // Check if we got an actual result or the model is still loading
            final decodedResponse = jsonDecode(response.body);
            
            // Check if the response indicates the model is still loading
            if (response.body.contains("loading") || response.body.contains("still loading") || 
                (decodedResponse is Map && decodedResponse.containsKey('error') && 
                 decodedResponse['error'].toString().toLowerCase().contains('loading'))) {
              
              debugPrint("Model is still loading, will retry");
              // This is a loading response, we should retry
              _currentRetry++;
              
              // Exponential backoff: wait longer with each retry
              int delaySeconds = _currentRetry * 2; // 2, 4, 6, 8, 10 seconds
              setState(() {
                _apiResult = "Model is loading... Waiting ${delaySeconds}s before retry ${_currentRetry}/${_maxRetries}";
              });
            ...
            // 503: Service unavailable - typically means the model is still loading
            // 429: Too many requests - rate limiting
            _currentRetry++;
            
            // Exponential backoff
            int delaySeconds = _currentRetry * 2;
            setState(() {
              String reason = response.statusCode == 503 ? "Model is loading" : "Rate limited";
              _apiResult = "$reason... Waiting ${delaySeconds}s before retry ${_currentRetry}/${_maxRetries}";
            });
            
            await Future.delayed(Duration(seconds: delaySeconds));
          } else {
            // Other error codes - we'll retry but log the specific error
            _currentRetry++;
            
            setState(() {
              _apiResult = "Error ${response.statusCode}: ${response.reasonPhrase}. Retrying (${_currentRetry}/${_maxRetries})...";
            });
            
            await Future.delayed(Duration(seconds: _currentRetry * 2));
          }
        } catch (e) {
          ...
          setState(() {
            _apiResult = "Connection error: ${e.toString().substring(0, min(100, e.toString().length))}. Retrying (${_currentRetry}/${_maxRetries})...";
          });
          
          await Future.delayed(Duration(seconds: _currentRetry));
        }
      }
      ...
      if (!success) {
        setState(() {
          _isProcessing = false;
          _apiResult = "Failed after ${_maxRetries} attempts. Please try again later.";
        });
      }
    } catch (e) {
      setState(() {
        _isProcessing = false;
        _apiResult = "Error processing audio: ${e.toString()}";
      });
    }
  }
