...
class QuranTextService {
  static Map<int, Map<int, String>> _quranText = {};
  ...
  static Future<void> initialize() async {
    ...
    try {
      final String rawData = await rootBundle.loadString('assets/data/Arabic-Original.csv');
      final List<List<dynamic>> csvTable = const CsvToListConverter().convert(rawData);
      
      // Skip header if exists
      int startIndex = csvTable[0][0] is int || csvTable[0][0].toString().contains(RegExp(r'^\d+$')) ? 0 : 1;
  ...
  static String? getAyah(int surahNo, int ayahNo) {
    if (!_isInitialized) {
      throw Exception('QuranTextService not initialized. Call initialize() first.');
    }
